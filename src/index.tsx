...
// from the items list
type Context = {
  registerItem: (item: HTMLSpanElement) => void
  removeItem: (item: HTMLSpanElement) => void
  registerKnob:  (item: HTMLSpanElement) => void
  removeKnob:  (item: HTMLSpanElement) => void
}
...
const SortableList = ({ children, allowDrag = true, onSortEnd, draggedItemClassName, ...rest }: Props) => {
  ...
  const registerItem = React.useCallback((item: HTMLSpanElement) => {
    itemsRef.current.push(item)
  }, [])

  const removeItem = React.useCallback((item: HTMLSpanElement) => {
  ...
  const registerKnob = React.useCallback((item: HTMLSpanElement) => {
    knobs.current.push(item)
  }, [])

  const removeKnob = React.useCallback((item: HTMLSpanElement) => {
  ...
  return (
    <span {...(allowDrag ? listeners : {})} {...rest} ref={containerRef}>
